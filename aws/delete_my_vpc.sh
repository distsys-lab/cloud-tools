...
for i in $vpcs
do
	sgs=`aws ec2 $option describe-security-groups --filters Name=vpc-id,Values=$i | jq -r .SecurityGroups[].GroupId`
	for j in $sgs
	do
		echo delete segurity group $j for vpc $i
		aws ec2 $option delete-security-group --group-id $j $is_dryrun
	done
done
#
# delete subnets
